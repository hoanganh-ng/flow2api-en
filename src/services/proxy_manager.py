...
class ProxyManager:
    ...
    def _parse_proxy_line(self, line: str) -> Optional[str]:
        """Convert user-provided proxy input into a standard URL format.
        ...
        if line.startswith(("http://", "https://", "socks5://", "socks5h://")):
            ...
            # Compatibility for protocol://host:port:user:pass
            try:
        ...
        # No protocol, with @: default to http
        if "@" in line:
            return f"http://{line}"

        # No protocol, decide based on colon count
        parts = line.split(":")
        if len(parts) == 2 and parts[1].isdigit():
            # host:port
    ...
    def normalize_proxy_url(self, proxy_url: Optional[str]) -> Optional[str]:
        """Normalize a proxy URL, returns None for empty, raises ValueError for invalid format."""
    ...
    async def get_proxy_url(self) -> Optional[str]:
        """Backwards-compatible: returns the request proxy URL"""
