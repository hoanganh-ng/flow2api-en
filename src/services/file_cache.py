...
class FileCache:
    ...
    def _get_request_fingerprint(self) -> Optional[Dict[str, Any]]:
        """Read the browser fingerprint bound to the current request context."""
    ...
    async def _resolve_download_proxy(
        ...
        """Resolve the download proxy URL based on media type."""
    ...
    def _guess_extension(self, url: str, media_type: str) -> str:
        """Preserve the original extension when possible, fall back to a default otherwise."""
    ...
    def _build_download_headers(
        ...
        """Build media download request headers, preferring the current captcha browser fingerprint."""
        headers = {
            "Accept": (
    ...
    def _write_cached_content(self, file_path: Path, content: bytes):
        """Write to a temp file first then atomically replace, so concurrent readers never see partial content."""
    ...
    def _normalize_cache_error(self, error: Exception) -> str:
        """Normalize cache errors so that raw command exceptions are not exposed to the user."""
