...
class LoadBalancer:
    ...
    async def _get_token_load(self, token_id: int, for_image_generation: bool, for_video_generation: bool) -> tuple[int, Optional[int]]:
        """Get the current load for a token.

        Returns:
            (inflight, remaining)
            remaining is None means unlimited
        """
    ...
    async def _reserve_slot(self, token_id: int, for_image_generation: bool, for_video_generation: bool) -> bool:
        """Try to pre-reserve a generation slot for the current token."""
        if not self.concurrency_manager:
            return True
    ...
    async def _check_extension_route(self, token: Token) -> tuple[bool, str]:
        ...
        try:
            ...
            return False, f"Extension route not configured or anonymous plugin not connected (available routes: {available})"
    ...
    async def select_token(
        ...
        for token in active_tokens:
            ...
            if model and not supports_model_for_tier(model, normalized_tier):
                filtered_reasons[token.id] = 'Account tier insufficient, requires ' + get_paygate_tier_label(required_tier)
                continue
            if for_image_generation:
        ...
        # Lowest in-flight first; when concurrency limits exist, tokens with more remaining slots win; random tiebreak
        ...
        debug_logger.log_info("[LOAD_BALANCER] Candidate token loads:")
        ...
        # Only run AT validation on the tokens we actually try, instead of scanning every token for every request
        for item in available_tokens:
            token = item["token"]
            token_id = token.id
            ...
            if reserve and not await self._reserve_slot(token.id, for_image_generation, for_video_generation):
                debug_logger.log_info(f"[LOAD_BALANCER] Skipping Token {token.id}: failed to pre-reserve slot")
                continue
    ...
    async def get_unavailable_reason(
        ...
        *,
        ...
        """Provide a more specific reason for "no available accounts" cases, primarily for resolution/tier hints."""
        active_tokens = await self.token_manager.get_active_tokens()
        if not active_tokens:
            return None
        ...
        if model and not supported_tokens:
            ...
            return f"Current model requires {tier_label} account, but no available {tier_label} account: {model}"
        ...
        if supported_tokens and not capability_tokens:
            if for_image_generation:
                return "Tier-matching accounts exist, but image generation is fully disabled."
            if for_video_generation:
                return "Tier-matching accounts exist, but video generation is fully disabled."
