...
def _extract_error_summary(payload: Any) -> str:
    """Extract a user-readable error summary from the response body."""
    if payload is None:
        return ""
...
def _guess_client_hints_from_user_agent(user_agent: str) -> Dict[str, str]:
    """Fill in common sec-ch-* headers based on UA."""
...
async def _prepare_captcha_runtime(method: str):
    ...
    try:
        if runtime_method == "browser":
            start_runtime_prepare(
                runtime_method,
                "Started preparing headed browser captcha runtime; installation progress will be displayed automatically.",
            )
        else:
            start_runtime_prepare(
                runtime_method,
                "Started preparing built-in browser captcha runtime; installation progress will be displayed automatically.",
            )
        ...
        if runtime_method == "browser":
            ...
            finish_runtime_prepare(runtime_method, "Chromium browser environment ready; headed browser captcha can be used.")
            return
        ...
        finish_runtime_prepare(runtime_method, "Built-in browser environment ready; personal captcha can be used.")
    except HTTPException:
        raise
    except Exception as e:
        fail_runtime_prepare(runtime_method, f"Browser environment preparation failed: {type(e).__name__}: {e}")
    finally:
...
def _schedule_captcha_runtime_prepare(method: str) -> bool:
    ...
    if task and not task.done():
        progress_runtime_prepare(runtime_method, "Browser environment preparation task is still in progress, please wait...")
        return False
...
def _guess_impersonate_from_user_agent(user_agent: str) -> str:
    """Pick a usable curl_cffi browser fingerprint version from UA."""
    ua = (user_agent or "").strip()
    major_match = re.search(r"(?:Chrome|Chromium|Edg|EdgA|EdgiOS)/(\d+)", ua)
    if not major_match:
        return "chrome120"
...
def _normalize_http_base_url(base_url: str) -> str:
    ...
    if not normalized:
        raise RuntimeError("Remote captcha service address is not configured")

    parsed = urlparse(normalized)
    if parsed.scheme not in {"http", "https"} or not parsed.netloc:
        raise RuntimeError("Remote captcha service address has invalid format; must be http(s)://host[:port]")
...
async def _sync_json_http_request(
    ...
    try:
        # remote_browser control plane is service-to-service JSON API; use httpx to avoid curl_cffi
        # dropping POST body under Windows + impersonate, which causes FastAPI to flag missing body.
...
async def _resolve_score_test_verify_proxy(
    ...
    """
    Select the proxy for score-test verify requests, preferring the browser captcha proxy.
    Returns: (proxies, used, source, proxy_url)
    """
    # For browser captcha mode, prefer browser_proxy to match token acquisition egress
    ...
    # Fall back to request proxy config
    try:
        if proxy_manager:
...
async def _solve_recaptcha_with_api_service(
    ...
    """Use the currently configured third-party captcha service to obtain a token."""
    ...
    async with AsyncSession() as session:
        ...
        for _ in range(40):
            ...
            if poll_json.get("status") == "ready":
                ...
                if token:
                    return token
                raise RuntimeError(f"{method} result missing token: {poll_json}")
    ...
    raise RuntimeError(f"{method} token fetch timed out")
...
async def _score_test_with_remote_browser_service(
    ...
    """Call remote headed captcha service to perform in-page captcha and score verification."""
    ...
    if not isinstance(response_payload, dict):
        raise RuntimeError("Remote captcha service returned invalid format")
...
async def get_tokens(token: str = Depends(verify_admin_token)):
    ...
    return [{
        ...
        "token": row.get("at"),  # Backward-compatible with frontend token.token access
        "email": row.get("email"),
        "name": row.get("name"),
    ...
    } for row in token_rows]  # Return array directly for frontend compatibility
...
async def add_token(
    ...
    try:
        ...
        # Hot-reload concurrency limits to avoid service restart
...
async def update_token(
    ...
    """Update token - automatically refresh AT via ST"""
    try:
        ...
        # Update token (includes AT, ST, AT expiration, project_id, project_name)
        await token_manager.update_token(
            token_id=token_id,
            st=request.st,
            at=at,
            at_expires=at_expires,  # Update AT expiration
        ...
        # Hot-reload concurrency limits to apply dashboard changes immediately
        if concurrency_manager:
...
async def refresh_at(
    ...
    """Manually refresh token's AT (via ST conversion)

    If AT refresh fails and personal mode is active, automatically attempts browser ST refresh
    """
    ...
    try:
        # Call token_manager's internal refresh method (includes ST auto-refresh logic)
        ...
        if success:
            ...
            if config.captcha_method == "personal":
                message += " (with ST auto-refresh support)"

            debug_logger.log_info(f"[API] AT refresh succeeded: token_id={token_id}")
            ...
            if config.captcha_method != "personal":
                error_detail += f" (current captcha mode: {config.captcha_method}; ST auto-refresh only available in personal mode)"

            raise HTTPException(status_code=500, detail=error_detail)
    except HTTPException:
        raise
    except Exception as e:
        debug_logger.log_error(f"[API] Exception refreshing AT: {str(e)}")
...
async def import_tokens(
    ...
    updated = 0
    errors = []
    # Maintain consistency with historical logic: prefer matching the latest record per email in created_at DESC order
    existing_by_email = {}
    ...
    for idx, item in enumerate(request.tokens):
        try:
            ...
            # Use ST to AT conversion to fetch user info
            try:
                ...
                if not email:
                    errors.append(f"Item {idx+1}: unable to fetch email info")
                    continue
                ...
                # Use email to check whether token already exists
    ...
    return {
        ...
        "message": f"Import complete: added {added}, updated {updated}" + (f", {len(errors)} failed" if errors else "")
    }
...
async def test_proxy_connectivity(
    ...
    """Test whether proxy can reach target site (default https://labs.google/)"""
    ...
    try:
        ...
        return {
            "success": ok,
            "message": "Proxy is available" if ok else f"Proxy is reachable, but target returned status code {status_code}",
            "test_url": test_url,
            "final_url": final_url,
...
async def update_captcha_config(
    ...
    try:
        ...
        return {"success": False, "message": "Remote captcha timeout must be an integer number of seconds"}
    try:
    ...
    try:
        ...
        return {"success": False, "message": "Reset count must be an integer; 0 disables it"}
...
async def test_captcha_score(
    ...
    """Use the current captcha method to obtain a token and submit to antcpt for score verification."""
    ...
    try:
        ...
        if captcha_method == "browser":
            ...
            if captcha_method == "capsolver" and "antcpt.com" in website_url:
                ...
                if token_value:
                    ...
                    return {
                        "success": True,
                        "message": "CapSolver does not support antcpt. Successfully verified connectivity with Google Labs",
                        "captcha_method": captcha_method,
                        "website_url": "https://labs.google/",
                        "website_key": "6LdsFiUsAAAAAIjVDZcuLhaHiDn5nnHVXVRQGeMV",
                        "action": "IMAGE_GENERATION",
                        ...
                        "score": 0.9,
                        "verify_result": {"success": True, "message": "Skipped score verification"},
                        "verify_request_meta": {},
                        "browser_proxy_enabled": browser_proxy_enabled,
        ...
        # Remote headed-captcha custom-score may complete verification in-page,
        # and some implementations do not explicitly return the token; fall back
        # to verify_result locally.
        ...
        if not token_value:
            return {
                "success": False,
                "message": "No reCAPTCHA token retrieved",
                "captcha_method": captcha_method,
                "website_url": website_url,
                "website_key": website_key,
                "action": action,
        ...
        return {
            ...
            "message": "Score verification passed" if verify_success else "Score verification failed",
            "captcha_method": captcha_method,
            "website_url": website_url,
            "website_key": website_key,
            "action": action,
