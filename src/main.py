...
async def lifespan(app: FastAPI):
    ...
    # Sync database config to memory on startup to avoid missing personal/browser-related runtime config.
    ...
    # Try to grab the token snapshot before the browser service starts; it's reused for concurrency management and warmup.
    ...
    if captcha_config.captcha_method == "personal":
        ...
        if warmed_slots:
            ...
            # When no active token is available, open a login window for manual setup
...
generation_handler = GenerationHandler(
    ...
    db,
    concurrency_manager,
    proxy_manager  # pass proxy_manager parameter
)
